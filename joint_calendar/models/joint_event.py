...
class joint_event(models.Model):
    ...
    def _default_res_model(self):
        ...
        """
        return self.env["ir.model"].sudo().search([("model", "=", "joint.event")], limit=1)
    ...
    def _inverse_event_properties(self):
        ...
        Should not take place at real refreshing to optimize performance (@see ctx no_parent_updates)
        """
        if not self._context.get("no_parent_updates"):
            for event in self:
                if event.res_reference and event.rule_id and event.res_reference._name != self._name:
                    ...
                    if rule.fields_company_id and not rule.fields_company_id.readonly \
                            and (item[rule.fields_company_id.name] != event.company_id): 
                        data.update({
                            rule.fields_company_id.name: event.company_id.id,
                        })                    
                    if data:
    ...
    def _inverse_res_reference(self):
        ...
        for event in self:
            if event.res_reference:
                values = {
                    "res_model_domain": event.res_reference._name,
    ...
    privacy = fields.Selection(
        ...
        required=True,
        default="public",
        help="""* Public - a joint event would be visible for everybody
* Private - a joint event would be visible only for attendees""",
    ...
    def read(self, fields_to_read=None, load="_classic_read"):
        """
        Re-write to add security check
        """
    ...
    def write(self, vals):
        """
        Re-write to add security check and add alarm management

        Methods:
         * _notify_next_joint_alarm
        """
