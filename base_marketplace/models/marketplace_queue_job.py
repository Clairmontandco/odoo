...
class MkQueueJob(models.Model):
    ...
    state = fields.Selection([('draft', 'Draft'), ('partial_processed', 'Partial Processed'), ('processed', 'Processed'), ('failed', 'Failed')], default='draft', compute="_compute_queue_line_counts_and_state", store=True)
    ...
    listing_count = fields.Integer(string='Listing Count', compute='_compute_queue_line_counts_and_state', compute_sudo=True)
    order_count = fields.Integer(string='Order Count', compute='_compute_queue_line_counts_and_state', compute_sudo=True)
