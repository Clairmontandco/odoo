...
class MkListing(models.Model):
    ...
    def get_odoo_product_variant_and_listing_item(self, mk_instance_id, variant_id, variant_barcode, variant_sku):
        ...
        if mk_instance_id.sync_product_with == 'barcode' and variant_barcode:
            if not listing_item_id:
                listing_item_id = mk_listing_item_obj.search([('product_id.barcode', '=', variant_barcode), ('mk_id', '=', variant_id), ('mk_instance_id', '=', mk_instance_id.id)], limit=1)
                if not listing_item_id:
                    listing_item_id = mk_listing_item_obj.search([('product_id.barcode', '=', variant_barcode), ('mk_id', '=', False), ('mk_instance_id', '=', mk_instance_id.id)], limit=1)
            if not listing_item_id:
        ...
        if mk_instance_id.sync_product_with == 'sku' and variant_sku:
            if not listing_item_id:
                listing_item_id = mk_listing_item_obj.search([('default_code', '=ilike', variant_sku), ('mk_id', '=', variant_id), ('mk_instance_id', '=', mk_instance_id.id)], limit=1)
            if not listing_item_id:
                listing_item_id = mk_listing_item_obj.search([('product_id.default_code', '=ilike', variant_sku), ('mk_id', '=', variant_id), ('mk_instance_id', '=', mk_instance_id.id)], limit=1)
                if not listing_item_id:
                    listing_item_id = mk_listing_item_obj.search([('product_id.default_code', '=ilike', variant_sku), ('mk_id', '=', False), ('mk_instance_id', '=', mk_instance_id.id)], limit=1)
            if not listing_item_id:
        ...
        if mk_instance_id.sync_product_with == 'barcode_or_sku':
            if variant_sku and not listing_item_id:
                listing_item_id = mk_listing_item_obj.search([('default_code', '=ilike', variant_sku), ('mk_id', '=', variant_id), ('mk_instance_id', '=', mk_instance_id.id)], limit=1)
                if not listing_item_id:
                    listing_item_id = mk_listing_item_obj.search([('default_code', '=ilike', variant_sku), ('mk_id', '=', False), ('mk_instance_id', '=', mk_instance_id.id)], limit=1)
    ...
    def get_mk_listing_item(self, mk_instance_id):
        ...
        query = """
                ...
                where {} state in ('partially_available', 'assigned', 'done', 'cancel') and sm.company_id = {} 
        """.format(mk_instance_id.id, where_clause, mk_instance_id.company_id.id)
        ...
        if mrp:
            query = """
                SELECT mkli.id FROM product_product AS pp 
                    JOIN mrp_bom AS mbom ON mbom.product_tmpl_id=pp.product_tmpl_id
                    JOIN mrp_bom_line AS mbl ON mbl.bom_id=mbom.id
                    JOIN stock_move AS sm ON sm.product_id=mbl.product_id
                    JOIN mk_listing_item AS mkli ON pp.id = mkli.product_id and mkli.is_listed = true and mkli.mk_instance_id = {}
                WHERE {} sm.company_id = {} and sm.state in ('partially_available', 'assigned','done', 'cancel')""".format(mk_instance_id.id, where_clause, mk_instance_id.company_id.id)
            self._cr.execute(query)
            result += [int(i[0]) for i in self._cr.fetchall()]
    ...
    def remove_extra_listing_item(self, mk_id_list):
        if mk_id_list:
            ...
            if need_to_remove_id_set:
                ...
                need_to_remove_listing_items = self.env['mk.listing.item'].search([('mk_id', 'in', need_to_remove_id_list)])
                if need_to_remove_listing_items:
                    listing_item_ids = need_to_remove_listing_items.mapped('mk_id')
                    need_to_remove_listing_items.unlink()
                    self.env['mk.log'].create_update_log(mk_instance_id=self.mk_instance_id, mk_log_id=mk_log_id, mk_log_line_dict={'success': [
                        {'log_message': 'IMPORT LISTING: {} listing item deleted.'.format(listing_item_ids), 'queue_job_line_id': queue_line_id and queue_line_id.id or False}]})
        return True
