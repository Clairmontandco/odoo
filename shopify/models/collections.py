...
class ShopifyCollection(models.Model):
    ...
    collection_condition_ids = fields.One2many("shopify.collection.condition.ts", "shopify_collection_id", string="Conditions")
    ...
    def sync_collection_products(self, collection, mk_instance_id=None):
        ...
        for product in collection.products():
            ...
            if not shopify_product_template_id:
                mk_listing_obj.shopify_import_listings(mk_instance_id, False, False, mk_listing_id=str(product.id))
                shopify_product_template_id = mk_listing_obj.search([('mk_id', '=', product.id), ('mk_instance_id', '=', mk_instance_id.id)])
            if not shopify_product_template_id:
    ...
    def create_update_collections(self, collection_dict, collection_type, mk_instance_id):
        ...
        if condition_list:
            ...
            for condition_dict in condition_list:
                odoo_shopify_collection_id.write({'collection_condition_ids': [(2, line_id.id, False) for line_id in odoo_shopify_collection_id.collection_condition_ids]})
    ...
    def import_shopify_collections(self, mk_instance_id):
        mk_listing_obj = self.env['mk.listing']
        mk_instance_id.connection_to_shopify()
        shopify_manual_collection_list = self.fetch_all_shopify_manual_collections()
        ...
        shopify_automated_collection_list = self.fetch_all_shopify_automated_collections()
    ...
    def prepare_collection_vals(self):
        ...
        if self.collection_type == 'automated':
            ...
            for condition_id in self.collection_condition_ids:
                column = condition_id.column
                if condition_id.column == 'variant_compare_at_price' and condition_id.relation in ['is_set', 'is_not_set']:
                    column = 'is_price_reduced'
                condition_list.append({'column': column, 'relation': condition_id.relation,
                                       'condition': condition_id.condition or ''})
    ...
    def export_automated_collection_ts(self):
        ...
        if result.errors and result.errors.errors:
            raise ValidationError(_(result.errors.errors))
    ...
    def prepare_update_collection_vals(self, new_collection):
        ...
        if self.collection_type == 'automated':
            ...
            for condition_id in self.collection_condition_ids:
                ...
                if condition_id.column == 'variant_compare_at_price' and condition_id.relation in ['is_set', 'is_not_set']:
                    column = 'is_price_reduced'
                condition_list.append({'column': column, 'relation': condition_id.relation, 'condition': condition_id.condition})
